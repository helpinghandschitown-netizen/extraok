...
try{
  ...
  const {targetId}=await send('Target.createTarget',{url:'http://127.0.0.1:4177/#home'});
  ...
  await evalJs(`location.href=${JSON.stringify(reviewLink)}`);await waitFor(`document.querySelector('#decision-form')`,'client review');
  ...
  await evalJs(`location.href=${JSON.stringify(receiptLink)}`);await waitFor(`location.hash.startsWith('#record=')`,'imported record');await waitFor(`document.querySelector('.status.approved')`,'approved status');
